...
frappe.provide("verp.crm");
verp.pre_sales.set_as_lost("Opportunity");
...
frappe.ui.form.on("Opportunity", {
	...
	customer_address: function (frm, cdt, cdn) {
		verp.utils.get_address_display(frm, "customer_address", "address_display", false);
	},

	contact_person: verp.utils.get_contact_details,
	...
	currency: function (frm) {
		let company_currency = verp.get_currency(frm.doc.company);
	...
	make_supplier_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "verp.crm.doctype.opportunity.opportunity.make_supplier_quotation",
			frm: frm,
		});
	},
	...
	make_request_for_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "verp.crm.doctype.opportunity.opportunity.make_request_for_quotation",
			frm: frm,
		});
	},
...
verp.crm.Opportunity = class Opportunity extends frappe.ui.form.Controller {
	onload() {
	...
	setup_queries() {
		...
		me.frm.set_query("customer_address", verp.queries.address_query);
		...
		if (me.frm.doc.opportunity_from == "Lead") {
			me.frm.set_query("party_name", verp.queries["lead"]);
		} else if (me.frm.doc.opportunity_from == "Customer") {
			me.frm.set_query("party_name", verp.queries["customer"]);
	...
	create_quotation() {
		frappe.model.open_mapped_doc({
			method: "verp.crm.doctype.opportunity.opportunity.make_quotation",
			frm: this.frm,
		});
	}
	...
	make_customer() {
		frappe.model.open_mapped_doc({
			method: "verp.crm.doctype.opportunity.opportunity.make_customer",
			frm: this.frm,
		});
	}

	show_notes() {
		const crm_notes = new verp.utils.CRMNotes({
			frm: this.frm,
	...
	show_activities() {
		const crm_activities = new verp.utils.CRMActivities({
			frm: this.frm,
...
extend_cscript(cur_frm.cscript, new verp.crm.Opportunity({ frm: cur_frm }));

cur_frm.cscript.item_code = function (doc, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.item_code) {
		return frappe.call({
			method: "verp.crm.doctype.opportunity.opportunity.get_item_details",
