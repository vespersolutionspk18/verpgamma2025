...
verp.taxes_and_totals = class TaxesAndTotals extends verp.payments {
	setup() {
	...
	initialize_taxes() {
		...
		$.each(this.frm.doc["taxes"] || [], function(i, tax) {
			...
			if (!this.discount_amount_applied) {
				verp.accounts.taxes.validate_taxes_and_charges(tax.doctype, tax.name);
				verp.accounts.taxes.validate_inclusive_tax(tax, this.frm);
			}
	...
	fetch_round_off_accounts() {
		...
		if (me.frm.doc.company) {
			frappe.call({
				"method": "verp.controllers.taxes_and_totals.get_round_off_applicable_accounts",
				"args": {
		...
		frappe.call({
			method: "verp.controllers.taxes_and_totals.get_rounding_tax_settings",
			callback: function(r) {
