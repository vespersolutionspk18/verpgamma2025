...
$.extend(verp.queries, {
	...
	item: function (filters) {
		var args = { query: "verp.controllers.queries.item_query" };
		if (filters) args["filters"] = filters;
		return args;
	},

	bom: function () {
		return { query: "verp.controllers.queries.bom" };
	},

	task: function () {
		return { query: "verp.projects.utils.query_task" };
	},
	...
	employee: function () {
		return { query: "verp.controllers.queries.employee_query" };
	},
	...
	get_filtered_dimensions: function (doc, child_fields, dimension, company) {
		...
		return {
			query: "verp.controllers.queries.get_filtered_dimensions",
			filters: {
				dimension: dimension,
				account: account,
				company: company,
...
verp.queries.setup_queries = function (frm, options, query_fn) {
	var me = this;
...
*/
verp.queries.setup_warehouse_query = function (frm) {
	frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
		var row = locals[cdt][cdn];
		var filters = verp.queries.warehouse(frm.doc);
		if (row.item_code) {
			$.extend(filters, { query: "verp.controllers.queries.warehouse_query" });
