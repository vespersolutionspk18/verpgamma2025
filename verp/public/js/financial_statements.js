frappe.provide("verp.financial_statements");
...
verp.financial_statements = {
	...
	formatter: function (value, row, column, data, default_formatter, filter) {
		...
		if (data && column.fieldname == "account") {
			...
			if (data.account) {
				column.link_onclick =
					"verp.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			}
			column.is_tree = true;
		}
	...
	onload: function (report) {
		...
		verp.financial_statements.filters = get_filters();

		let fiscal_year = verp.utils.get_fiscal_year(frappe.datetime.get_today());
...
function get_filters() {
	let filters = [
		...
		// accordingly.
		{
			...
			label: __("Currency"),
			fieldtype: "Select",
			options: verp.get_presentation_currency_list(),
		},
		{
	...
	let fiscal_year = verp.utils.get_fiscal_year(frappe.datetime.get_today(), false, true);
	if (fiscal_year) {
		let fy = verp.utils.get_fiscal_year(frappe.datetime.get_today(), false, false);
