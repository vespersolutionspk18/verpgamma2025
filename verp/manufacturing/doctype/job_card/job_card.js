...
frappe.ui.form.on("Job Card", {
	setup: function (frm) {
		frm.set_query("operation", function () {
			return {
				query: "verp.manufacturing.doctype.job_card.job_card.get_operations",
				filters: {
	...
	make_corrective_job_card: function (frm, operation, for_operation) {
		frappe.call({
			method: "verp.manufacturing.doctype.job_card.job_card.make_corrective_job_card",
			args: {
	...
	operation: function (frm) {
		...
		if (frm.doc.operation && frm.doc.work_order) {
			frappe.call({
				method: "verp.manufacturing.doctype.job_card.job_card.get_operation_details",
				args: {
					work_order: frm.doc.work_order,
	...
	make_time_log: function (frm, args) {
		...
		frappe.call({
			method: "verp.manufacturing.doctype.job_card.job_card.make_time_log",
			args: {
				args: args,
			},
			freeze: true,
	...
	make_material_request: function (frm) {
		frappe.model.open_mapped_doc({
			method: "verp.manufacturing.doctype.job_card.job_card.make_material_request",
			frm: frm,
	...
	make_stock_entry: function (frm) {
		frappe.model.open_mapped_doc({
			method: "verp.manufacturing.doctype.job_card.job_card.make_stock_entry",
			frm: frm,
			run_link_triggers: true,
		});
	},
