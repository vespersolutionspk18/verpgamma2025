...
frappe.ui.form.on("Company", {
	...
	setup: function (frm) {
		...
		verp.company.setup_queries(frm);
	...
	refresh: function (frm) {
		...
		verp.company.set_chart_of_accounts_options(frm.doc);
	},
	...
	delete_company_transactions: function (frm) {
		frappe.call({
			method: "verp.setup.doctype.transaction_deletion_record.transaction_deletion_record.is_deletion_doc_running",
			args: {
				company: frm.doc.name,
			},
			freeze: true,
					...
					frappe.verify_password(function () {
						var d = frappe.prompt(
							...
							function (data) {
								...
								frappe.call({
									method: "verp.setup.doctype.company.company.create_transaction_deletion_request",
									args: {
...
verp.company.set_chart_of_accounts_options = function (doc) {
	var selected_value = doc.chart_of_accounts;
	if (doc.country) {
		return frappe.call({
			method: "verp.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
			args: {
				country: doc.country,
				with_standard: true,
			},
...
verp.company.setup_queries = function (frm) {
	$.each(
		[
		...
		function (i, v) {
			verp.company.set_custom_query(frm, v);
		}
	);
...
verp.company.set_custom_query = function (frm, v) {
	var filters = {
		company: frm.doc.name,
		is_group: 0,
	};
