...
# imported to enable verp.accounts.utils.get_account_currency
from verp.accounts.doctype.account.account import get_account_currency
...
from verp.stock.utils import get_stock_value_on

if TYPE_CHECKING:
	from verp.stock.doctype.repost_item_valuation.repost_item_valuation import RepostItemValuation
...
def reconcile_against_document(
	...
	for key, entries in reconciled_entries.items():
		...
		if voucher_type == "Payment Entry" and doc.book_advance_payments_in_separate_party_account:
			...
			from verp.accounts.general_ledger import process_debit_credit_difference
...
def get_outstanding_invoices(
	...
	if account:
		...
		party_account_type = verp.get_party_account_type(party_type)
...
def get_children(doctype, parent, company, is_root=False, include_disabled=False):
	...
	from verp.accounts.report.financial_statements import sort_accounts
...
def create_payment_gateway_account(gateway, payment_channel="Email"):
	from verp.setup.setup_wizard.operations.install_fixtures import create_bank_account
...
def get_coa(doctype, parent, is_root=None, chart=None):
	from verp.accounts.doctype.account.chart_of_accounts.chart_of_accounts import (
...
def repost_gle_for_stock_vouchers(
	...
	from verp.accounts.general_ledger import toggle_debit_credit_if_negative
	...
	journal_account = frappe._dict(
		{
			...
			"exchange_rate": 0,
			"cost_center": cost_center or verp.get_default_cost_center(company),
