...
import verp
from verp.accounts.deferred_revenue import validate_service_stop_date
from verp.accounts.doctype.loyalty_program.loyalty_program import (
	get_loyalty_program_details_with_points,
	validate_loyalty_points,
)
from verp.accounts.doctype.pricing_rule.utils import (
	update_coupon_code_count,
	validate_coupon_code,
)
from verp.accounts.doctype.repost_accounting_ledger.repost_accounting_ledger import (
	validate_docs_for_deferred_accounting,
	validate_docs_for_voucher_types,
)
from verp.accounts.doctype.tax_withholding_category.tax_withholding_category import (
	get_party_tax_withholding_details,
)
from verp.accounts.general_ledger import get_round_off_account_and_cost_center
from verp.accounts.party import get_due_date, get_party_account, get_party_details
from verp.accounts.utils import cancel_exchange_gain_loss_journal, get_account_currency
from verp.assets.doctype.asset.depreciation import (
	depreciate_asset,
...
from verp.assets.doctype.asset_activity.asset_activity import add_asset_activity
from verp.controllers.accounts_controller import validate_account_head
from verp.controllers.selling_controller import SellingController
from verp.projects.doctype.timesheet.timesheet import get_projectwise_timesheet_data
from verp.setup.doctype.company.company import update_company_current_month_sales
from verp.stock.doctype.delivery_note.delivery_note import update_billed_amount_based_on_so
...
class SalesInvoice(SellingController):
	...
	if TYPE_CHECKING:
		...
		from verp.accounts.doctype.payment_schedule.payment_schedule import PaymentSchedule
		from verp.accounts.doctype.pricing_rule_detail.pricing_rule_detail import PricingRuleDetail
		from verp.accounts.doctype.sales_invoice_advance.sales_invoice_advance import SalesInvoiceAdvance
		from verp.accounts.doctype.sales_invoice_item.sales_invoice_item import SalesInvoiceItem
		from verp.accounts.doctype.sales_invoice_payment.sales_invoice_payment import SalesInvoicePayment
		from verp.accounts.doctype.sales_invoice_timesheet.sales_invoice_timesheet import (
			SalesInvoiceTimesheet,
		)
		from verp.accounts.doctype.sales_taxes_and_charges.sales_taxes_and_charges import (
			SalesTaxesandCharges,
		)
		from verp.selling.doctype.sales_team.sales_team import SalesTeam
		from verp.stock.doctype.packed_item.packed_item import PackedItem
	...
	def check_credit_limit(self):
		from verp.selling.doctype.customer.customer import check_credit_limit
	...
	def set_pos_fields(self, for_validate=False):
		...
		from verp.stock.get_item_details import get_pos_profile, get_pos_profile_item_details
	...
	def update_packing_list(self):
		if cint(self.update_stock) == 1:
			from verp.stock.doctype.packed_item.packed_item import make_packing_list
	...
	def make_gl_entries(self, gl_entries=None, from_repost=False):
		from verp.accounts.general_ledger import make_gl_entries, make_reverse_gl_entries

		auto_accounting_for_stock = verp.is_perpetual_inventory_enabled(self.company)
		if not gl_entries:
		...
		if gl_entries:
			...
			if update_outstanding == "No":
				from verp.accounts.doctype.gl_entry.gl_entry import update_outstanding_amt
	...
	def get_gl_entries(self, warehouse_account=None):
		from verp.accounts.general_ledger import merge_similar_entries
	...
	def make_item_gl_entries(self, gl_entries):
		...
		if cint(self.update_stock) and verp.is_perpetual_inventory_enabled(self.company):
	...
	def apply_loyalty_points(self):
		from verp.accounts.doctype.loyalty_point_entry.loyalty_point_entry import (
...
def get_list_context(context=None):
	from verp.controllers.website_list_for_contact import get_list_context
...
def get_loyalty_programs(customer):
	...
	from verp.selling.doctype.customer.customer import get_loyalty_programs
...
def create_dunning(source_name, target_doc=None, ignore_permissions=False):
	...
	def postprocess_dunning(source, target):
		from verp.accounts.doctype.dunning.dunning import get_dunning_letter_text
