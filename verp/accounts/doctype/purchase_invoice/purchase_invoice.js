...
verp.accounts.payment_triggers.setup("Purchase Invoice");
verp.accounts.taxes.setup_tax_filters("Purchase Taxes and Charges");
verp.accounts.taxes.setup_tax_validations("Purchase Invoice");
verp.buying.setup_buying_controller();

verp.accounts.PurchaseInvoice = class PurchaseInvoice extends verp.buying.BuyingController {
	setup(doc) {
	...
	unblock_invoice() {
		const me = this;
		frappe.call({
			method: "verp.accounts.doctype.purchase_invoice.purchase_invoice.unblock_invoice",
	...
	make_comment_dialog_and_block_invoice() {
		...
		this.dialog.set_primary_action(__("Save"), function () {
			...
			frappe.call({
				method: "verp.accounts.doctype.purchase_invoice.purchase_invoice.block_invoice",
				args: {
	...
	set_release_date(data) {
		return frappe.call({
			method: "verp.accounts.doctype.purchase_invoice.purchase_invoice.change_release_date",
			args: data,
	...
	make_inter_company_invoice(frm) {
		frappe.model.open_mapped_doc({
			method: "verp.accounts.doctype.purchase_invoice.purchase_invoice.make_inter_company_sales_invoice",
			frm: frm,
		});
	}
	...
	make_debit_note() {
		frappe.model.open_mapped_doc({
			method: "verp.accounts.doctype.purchase_invoice.purchase_invoice.make_debit_note",
			frm: this.frm,
		});
	}
};

cur_frm.script_manager.make(verp.accounts.PurchaseInvoice);
...
cur_frm.fields_dict["items"].grid.get_field("item_code").get_query = function (doc, cdt, cdn) {
	return {
		query: "verp.controllers.queries.item_query",
...
frappe.ui.form.on("Purchase Invoice", {
	...
	mode_of_payment: function (frm) {
		verp.accounts.pos.get_payment_mode_account(frm, frm.doc.mode_of_payment, function (account) {
