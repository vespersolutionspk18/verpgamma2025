...
from verp.accounts.doctype.pricing_rule.pricing_rule import (
...
from verp.stock.doctype.item.item import get_item_defaults, get_uom_conv_factor
from verp.stock.doctype.item_manufacturer.item_manufacturer import get_item_manufacturer_part_no
from verp.stock.doctype.price_list.price_list import get_price_list_details
...
def get_basic_details(args, item, overwrite_warehouse=True):
	...
	if args.get("doctype") in purchase_doctypes and not frappe.db.get_single_value(
		...
		from verp.buying.doctype.purchase_order.purchase_order import item_last_purchase_rate
...
def get_default_cost_center(args, item=None, item_group=None, brand=None, company=None):
	...
	if not cost_center and (item and item_group and brand):
		...
		for method in ["get_item_defaults", "get_item_group_defaults", "get_brand_defaults"]:
			path = f"verp.stock.get_item_details.{method}"
...
def get_price_list_rate(args, item_doc, out=None):
	...
	if meta.get_field("currency") or args.get("currency"):
		...
		if not out.price_list_rate and args.transaction_type == "buying":
			from verp.stock.doctype.item.item import get_last_purchase_details
...
def validate_conversion_rate(args, meta):
	from verp.controllers.accounts_controller import validate_conversion_rate
...
def get_batch_qty(batch_no, warehouse, item_code):
	from verp.stock.doctype.batch import batch
