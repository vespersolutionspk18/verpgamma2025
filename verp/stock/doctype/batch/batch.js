...
frappe.ui.form.on("Batch", {
	...
	make_dashboard: (frm) => {
		if (!frm.is_new()) {
			...
			frappe.call({
				method: "verp.stock.doctype.batch.batch.get_batch_qty",
				...
				callback: (r) => {
					...
					rows.find(".btn-move").on("click", function () {
						...
						frappe.prompt(
							...
							(data) => {
								frappe.call({
									method: "verp.stock.doctype.stock_entry.stock_entry_utils.make_stock_entry",
									args: {
					...
					rows.find(".btn-split").on("click", function () {
						...
						frappe.prompt(
							...
							(data) => {
								frappe
									.xcall("verp.stock.doctype.batch.batch.split_batch", {
										item_code: frm.doc.item,
										batch_no: frm.doc.name,
										qty: data.qty,
