...
import verp
from verp.controllers.item_variant import (
...
from verp.stock.doctype.item_default.item_default import ItemDefault
...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from verp.stock.doctype.item_barcode.item_barcode import ItemBarcode
		from verp.stock.doctype.item_customer_detail.item_customer_detail import ItemCustomerDetail
		from verp.stock.doctype.item_default.item_default import ItemDefault
		from verp.stock.doctype.item_reorder.item_reorder import ItemReorder
		from verp.stock.doctype.item_supplier.item_supplier import ItemSupplier
		from verp.stock.doctype.item_tax.item_tax import ItemTax
		from verp.stock.doctype.item_variant_attribute.item_variant_attribute import ItemVariantAttribute
		from verp.stock.doctype.uom_conversion_detail.uom_conversion_detail import UOMConversionDetail
			...
			item_price = frappe.get_doc(
				{
					...
					"uom": self.stock_uom,
					"brand": self.brand,
					"currency": verp.get_default_currency(),
			...
			for item_barcode in self.barcodes:
				...
				if item_barcode.barcode:
					...
					if item_barcode.barcode_type:
						barcode_type = convert_verp_to_barcodenumber(
	...
	def recalculate_bin_qty(self, new_name):
		from verp.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"verp.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
...
def convert_verp_to_barcodenumber(verp_number, barcode):
	if verp_number == "EAN":
		ean_type = {
			8: "EAN8",
			13: "EAN13",
	...
	if verp_number in convert:
		return convert[verp_number]
...
def get_asset_naming_series():
	from verp.assets.doctype.asset.asset import get_asset_naming_series
...
def get_child_warehouses(warehouse):
	from verp.stock.doctype.warehouse.warehouse import get_child_warehouses
