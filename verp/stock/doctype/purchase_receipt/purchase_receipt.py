...
import verp
from verp.accounts.utils import get_account_currency
from verp.assets.doctype.asset.asset import get_asset_account, is_cwip_accounting_enabled
...
from verp.controllers.accounts_controller import merge_taxes
from verp.controllers.buying_controller import BuyingController
from verp.stock.doctype.delivery_note.delivery_note import make_inter_company_transaction
...
class PurchaseReceipt(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from verp.stock.doctype.purchase_receipt_item.purchase_receipt_item import PurchaseReceiptItem
	...
	def make_item_gl_entries(self, gl_entries, warehouse_account=None):
		...
		for d in self.get("items"):
			if (
				...
				if not (
					(verp.is_perpetual_inventory_enabled(self.company) and d.item_code in stock_items)
					or d.is_fixed_asset
				):
					continue
...
def make_purchase_invoice(source_name, target_doc=None, args=None):
	from verp.accounts.party import get_payment_terms_template
