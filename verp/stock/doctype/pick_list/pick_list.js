...
frappe.ui.form.on("Pick List", {
	setup: (frm) => {
		...
		frm.set_query("work_order", () => {
			return {
				query: "verp.stock.doctype.pick_list.pick_list.get_pending_work_orders",
				filters: {
					company: frm.doc.company,
				},
			};
	...
	refresh: (frm) => {
		...
		if (frm.doc.docstatus === 1) {
			frappe
				.xcall("verp.stock.doctype.pick_list.pick_list.target_document_exists", {
				...
				frappe.prompt(
					...
					(data) => {
						...
						verp.utils.map_current_doc({
							method: "verp.manufacturing.doctype.work_order.work_order.create_pick_list",
							target: frm,
	...
	create_delivery_note: (frm) => {
		frappe.model.open_mapped_doc({
			method: "verp.stock.doctype.pick_list.pick_list.create_delivery_note",
			frm: frm,
		});
	},
	create_stock_entry: (frm) => {
		frappe
			.xcall("verp.stock.doctype.pick_list.pick_list.create_stock_entry", {
				pick_list: frm.doc,
			})
			.then((stock_entry) => {
	...
	scan_barcode: (frm) => {
		...
		const barcode_scanner = new verp.utils.BarcodeScanner(opts);
...
function get_item_details(item_code, uom = null) {
	if (item_code) {
		return frappe.xcall("verp.stock.doctype.pick_list.pick_list.get_item_details", {
			item_code,
			uom,
		});
	}
