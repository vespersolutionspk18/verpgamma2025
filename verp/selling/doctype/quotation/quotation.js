...
verp.accounts.taxes.setup_tax_validations("Sales Taxes and Charges Template");
verp.accounts.taxes.setup_tax_filters("Sales Taxes and Charges");
verp.pre_sales.set_as_lost("Quotation");
...
verp.selling.QuotationController = class QuotationController extends verp.selling.SellingController {
	onload(doc, dt, dn) {
	...
	set_dynamic_field_label() {
		if (this.frm.doc.quotation_to == "Customer") {
			...
			this.frm.fields_dict.party_name.get_query = function () {
				return { query: "verp.controllers.queries.lead_query" };
			};
	...
	get_lead_details() {
		...
		frappe.call({
			method: "verp.crm.doctype.lead.lead.get_lead_details",
			args: {
...
cur_frm.script_manager.make(verp.selling.QuotationController);
