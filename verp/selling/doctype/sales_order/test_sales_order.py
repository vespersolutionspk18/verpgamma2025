...
from verp.controllers.accounts_controller import InvalidQtyError, update_child_qty_rate
from verp.maintenance.doctype.maintenance_schedule.test_maintenance_schedule import (
	make_maintenance_schedule,
)
from verp.maintenance.doctype.maintenance_visit.test_maintenance_visit import (
	make_maintenance_visit,
)
from verp.manufacturing.doctype.blanket_order.test_blanket_order import make_blanket_order
from verp.selling.doctype.product_bundle.test_product_bundle import make_product_bundle
from verp.selling.doctype.sales_order.sales_order import (
	WarehouseRequired,
	create_pick_list,
	make_delivery_note,
...
class TestSalesOrder(FrappeTestCase):
	...
	def test_drop_shipping(self):
		from verp.buying.doctype.purchase_order.purchase_order import update_status
		from verp.selling.doctype.sales_order.sales_order import (
			make_purchase_order_for_default_supplier,
		)
		from verp.selling.doctype.sales_order.sales_order import update_status as so_update_status
	...
	def test_product_bundles_in_so_are_replaced_with_bundle_items_in_po(self):
		...
		"""
		from verp.selling.doctype.sales_order.sales_order import make_purchase_order
	...
	def test_make_work_order(self):
		from verp.selling.doctype.sales_order.sales_order import get_work_order_items
	...
	def test_work_order_pop_up_from_sales_order(self):
		...
		from verp.controllers.item_variant import create_variant
		from verp.manufacturing.doctype.production_plan.test_production_plan import make_bom
		from verp.selling.doctype.sales_order.sales_order import get_work_order_items
	...
	def test_so_cancellation_after_work_order_submission(self):
		...
		Expected result: Sales Order should not get cancelled
		"""
		from verp.manufacturing.doctype.work_order.test_work_order import make_wo_order_test_record
	...
	def test_so_back_updated_from_wo_via_mr(self):
		...
		from verp.manufacturing.doctype.work_order.work_order import (
			make_stock_entry as make_se_from_wo,
		)
		from verp.stock.doctype.material_request.material_request import raise_work_orders
	...
	@patch(
		...
		"verp.accounts.doctype.payment_request.payment_request.PaymentRequest.get_payment_url",
		return_value=None,
	)
	def test_sales_order_advance_payment_status(self, mocked_get_payment_url):
		from verp.accounts.doctype.payment_entry.test_payment_entry import get_payment_entry
		from verp.accounts.doctype.payment_request.payment_request import make_payment_request
	...
	def test_pick_list_for_batch(self):
		from verp.stock.doctype.pick_list.pick_list import create_delivery_note
