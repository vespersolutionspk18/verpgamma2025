frappe.listview_settings["Sales Order"] = {
	...
	onload: function (listview) {
		var method = "verp.selling.doctype.sales_order.sales_order.close_or_unclose_sales_orders";
		...
		listview.page.add_action_item(__("Sales Invoice"), () => {
			verp.bulk_transaction_processing.create(listview, "Sales Order", "Sales Invoice");
		});

		listview.page.add_action_item(__("Delivery Note"), () => {
			frappe.call({
				method: "verp.selling.doctype.sales_order.sales_order.is_enable_cutoff_date_on_bulk_delivery_note_creation",
				callback: (r) => {
					if (r.message) {
						...
						dialog.set_primary_action(__("Select"), function (values) {
							...
							verp.bulk_transaction_processing.create(
								listview,
								"Sales Order",
								"Delivery Note",
								{
						...
						verp.bulk_transaction_processing.create(listview, "Sales Order", "Delivery Note");
					}
				},
			});
		});
		...
		listview.page.add_action_item(__("Advance Payment"), () => {
			verp.bulk_transaction_processing.create(listview, "Sales Order", "Payment Entry");
		});
	},
};
